...
async function renderDashboard() {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        div.innerHTML = `
            ...
            <div class="t-info">
                <h4>Domingo</h4>
                <p>${plan ? (escapeHtml(plan.presiding) || 'Rascunho') : 'Não planeado'}</p>
            </div>
    ...
    state.speakers.forEach((item, index) => {
        ...
        div.innerHTML = `
            <div class="input-wrapper">
                ...
                <input type="text" class="plan-speaker-input" data-id="${item.id}" value="${escapeHtml(item.name || '')}" placeholder="Nome do membro...">
            </div>
    ...
    state.speakers.forEach((item, index) => {
        ...
        if (item.type === 'speaker') {
            ...
            div.innerHTML = `
                 <div class="input-wrapper">
                    <label>Orador ${index + 1}</label>
                    <input type="text" class="speaker-name-input" data-id="${item.id}" value="${escapeHtml(item.name || '')}" placeholder="Nome do membro...">
                </div>
                <!-- Remove button -->
                <button class="btn btn-danger btn-remove-speaker" data-id="${item.id}" style="position:absolute; top:10px; right:10px; padding:4px 8px; font-size:0.75rem;">×</button>
            `;
...
async function renderRoster() {
    ...
    list.slice(0, 50).forEach(m => {
        ...
        tr.innerHTML = `
            <td>
                <div class="view-link" data-id="${m.id}" style="font-weight:600; cursor:pointer;">${escapeHtml(m.name)}</div>
